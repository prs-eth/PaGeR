...
from src.depth_anything_3.model.utils.valid_conv_padding import cube_resample_pad
...
def custom_interpolate(
    ...
    # Pad-then-interpolate-then-crop keeps the 1-pixel ring of upsample samples
    # anchored in geometrically-correct neighbour-face content rather than
    # zero-padded territory. Uses the spherical resample pad (same method as
    # the valid-conv padding installed by set_valid_pad_conv).
    if valid_conv_padding:
        x = cube_resample_pad(x, padding=1)
        x = nn.functional.interpolate(x, size=(size[0]+2, size[1]+2), mode=mode, align_corners=align_corners)
