...
class DinoVisionTransformer(nn.Module):
    def __init__(
        ...
        # Released PaGeR checkpoints all run with the same backbone flags:
        # no global attention rope, no global per-face positional encoding,
        # no seam-local attention, no RoPE-LoRA adapters, no gradient
        # checkpointing at inference. The corresponding branches have been
        # excised so the forward path here describes only what we ship.
        self._latlon_cache = {}
    ...
    def prepare_tokens_with_masks(self, x, masks=None, cls_token=None, face_ids=None, **kwargs):
        B, S, nc, w, h = x.shape
    ...
    def _prepare_rope(self, B, S, H, W, device, face_ids=None):
        """Build positions for RoPE.

        Released PaGeR runs ``global_attn_rope=False`` and ``rope_lora_rank=0``,
        so we only need the direct local-attention path: integer patch
        positions for local blocks, plus a constant-zero ``pos_nodiff`` (the
        camera token sits at ``(0, 0)``, patches shift to ``(1, 1)`` to keep
        the camera at the identity-rotation anchor).
        """
        if self.rope is None:
            return None, None, None

        grid_h, grid_w = H // self.patch_size, W // self.patch_size
        pos = self.position_getter(B * S, grid_h, grid_w, device=device)
        pos = rearrange(pos, "(b s) n c -> b s n c", b=B)
        pos_nodiff = torch.zeros_like(pos)

        if self.patch_start_idx > 0:
            pos = pos + 1
            pos_special_int = torch.zeros(
                B, S, self.patch_start_idx, 2, device=device, dtype=pos.dtype,
            )
            pos = torch.cat([pos_special_int, pos], dim=2)
            pos_nodiff = pos_nodiff + 1
            pos_nodiff = torch.cat([pos_special_int, pos_nodiff], dim=2)

        return pos, pos_nodiff, None

    def _get_intermediate_layers_not_chunked(self, x, n=1, export_feat_layers=[], face_ids=None, **kwargs):
        B, S, _, H, W = x.shape
        x = self.prepare_tokens_with_masks(x, face_ids=face_ids)
        output, total_block_len = [], len(self.blocks)
        blocks_to_take = range(total_block_len - n, total_block_len) if isinstance(n, int) else n
        pos, pos_nodiff, _ = self._prepare_rope(B, S, H, W, x.device, face_ids=face_ids)
        ...
        for i, blk in enumerate(self.blocks):
            ...
            # ``cam_token`` is supplied by the camera encoder for every PaGeR
            # forward pass; reference-view selection / fallback tokens are not
            # exercised here, so the corresponding branches have been removed.
            if self.alt_start != -1 and i == self.alt_start:
                x[:, :, 0] = kwargs["cam_token"]

            use_global = self.alt_start != -1 and i >= self.alt_start and i % 2 == 1
            curr_attn_type = "global" if use_global else "local"
            curr_pos = g_pos if use_global else l_pos
            curr_attn_mask = kwargs.get("attn_mask", None) if use_global else None

            x = self.process_attention(
                x, blk, curr_attn_type, pos=curr_pos, attn_mask=curr_attn_mask,
            )
